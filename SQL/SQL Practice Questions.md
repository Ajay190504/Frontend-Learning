...
SELECT \* FROM orders WHERE payment\_mode = 'UPI';
...
SELECT \* FROM orders WHERE category = 'Electronics' OR category = 'Clothing';
...
SELECT \* FROM orders WHERE product\_name = 'Phone';
...
SELECT \* FROM orders ORDER BY order\_date DESC;



**11) Find the total revenue (price × quantity)**

SELECT SUM(price \* quantity) AS total\_revenue FROM orders;



**12) Find average price of products for each category**

SELECT category, AVG(price) AS avg\_price FROM orders GROUP BY category;



**13) Count total number of orders**

SELECT COUNT(\*) AS total\_orders FROM orders;
